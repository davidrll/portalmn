...
class Model_portal extends CI_Model {
    ...
    #buscar noticia por su titulo
    public function getBuscarNoticia(){
        $search = $_GET['term']; 
        $query = $this->db->query("SELECT codigo AS value, titulo AS label FROM noticias WHERE titulo LIKE '%$search%'");
        if($query->num_rows()>0)
        {
            return $query->result();
        }
    ...
    function get_bird($q){
        $this->db->select('codigo AS value, titulo AS label, sumilla_g AS image, fecha');
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                ...
                $new_row['fecha']=htmlentities(stripslashes($row['fecha']));
...
#Trae Video de Recomendaciones
    public function getEspecRecom(){
        $query = $this->db->query("SELECT * FROM columnista WHERE flg_recom=1 AND estado_colum=1");
        if($query->num_rows()>0)
        {
            return $query->result();
        }
    ...
    public function getNoticiaRelac($it){
        ...
        if(empty($it)){
            ...
            $query = $this->db->query("SELECT noticias.codigo,noticias.subida,noticias.titulo,noticias.sumilla_g AS portada,SUBSTR(REPLACE(fnStripTags(noticias.contenido),'&nbsp;',''),1,190) AS extracto,DATE(noticias.fecha) AS fecha FROM noticias WHERE noticias.estado = 1 AND noticias.codigo <> ".$idnoti." AND noticias.fecha between DATE('2014-01-01') and DATE('2014-12-31')".$filtro." ORDER BY fecha DESC LIMIT 5");
            if($query->num_rows()>0)
            {
                return $query->result();
            }
    ...
    #Trae Items de la Nota del Especialista Mercado Negro
    public function getNotaRelac($it){
        $filtro="";
        //$i=0;
        if(empty($it)){
            return 0;
            ...
            $nombre = str_replace('-', ' ', $urlamigable);
            $idesp = $this->security->xss_clean($this->uri->segment(2));
            $query = $this->db->query("SELECT columnista.codigo,columnista.nombres,columnista.frase,columnista.imag_colum AS portada,SUBSTR(REPLACE(fnStripTags(columnista.desc_colum),'&nbsp;',''),1,190) AS extracto,DATE(columnista.fecha_colum) AS fecha FROM columnista WHERE columnista.nombres = "."'$nombre'");
            if($query->num_rows()>0)
            {
                return $query->result();
            }
    ...
    #Primer Bloque de Noticias
    public function getBloqueNoticiasRecientes($bloque = 1){
        if($bloque == 1) {
            $limit = 'LIMIT 6';
        }elseif ($bloque == 2) {
            $limit = 'LIMIT 6, 11';
        ...
        //id de la noticia actual
        $idnoti = $this->security->xss_clean($this->uri->segment(2));
        $query = $this->db->query("SELECT noticias.codigo,noticias.subida,noticias.titulo,noticias.sumilla_g AS portada,SUBSTR(REPLACE(fnStripTags(noticias.contenido),'&nbsp;',''),1,190) AS extracto,DATE(noticias.fecha) AS fecha FROM noticias WHERE noticias.estado = 1 AND noticias.codigo <> ".$idnoti." ORDER BY noticias.fecha DESC ".$limit);
        if($query->num_rows()>0)
        {
            return $query->result();
        }
    ...
    /*
    #Primer Bloque de Especialista
    public function getBloqueEspecialista($bloque = 1){
        if($bloque == 1) {
            $limit = 'LIMIT 6';
        }elseif ($bloque == 2) {
            $limit = 'LIMIT 6, 11';
        ...
        //id de sus notas
        $idesp = $this->security->xss_clean($this->uri->segment(2));
        $query = $this->db->query("SELECT columnista.codigo,columnista.nombres,columnista.frase,columnista.imag_colum AS portada,SUBSTR(REPLACE(fnStripTags(columnista.desc_colum),'&nbsp;',''),1,190) AS extracto,DATE(columnista.fecha_colum) AS fecha FROM columnista WHERE columnista.estado_colum = 1 AND columnista.codigo <> ".$idesp." ORDER BY columnista.fecha_colum DESC ".$limit);
        if($query->num_rows()>0)
        {
            return $query->result();
        }
    ...
    public function getEspecialistas(){
        $query = $this->db->query("SELECT codigo,nombres,cargo,imag_colum AS fotoesp,frase AS titulocolum, TRIM(fnStripTags(desc_colum)) AS desc_colum FROM columnista WHERE estado_colum = 1 ORDER BY fecha_colum DESC LIMIT 6");
        if($query->num_rows()>0)
        {
            return $query->result();
        }
    ...
    public function getTodosColumnistas(){
        $query = $this->db->query("SELECT columnista.codigo,columnista.nombres,SUBSTR(fnStripTags(columnista.bio),1,250) AS bio,columnista.cargo,columnista.imag_colum,columnista.estado_colum,frase AS titulocolum,
        TRIM(fnStripTags(desc_colum)) AS desc_colum FROM columnista WHERE columnista.estado_colum = 1 AND YEAR(columnista.fecha_colum) = YEAR(CURDATE()) ORDER BY columnista.fecha_colum DESC");
        if($query->num_rows()>0)
        {
            return $query->result();
        }
