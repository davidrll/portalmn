		    		...
		    		if($("#mail_cont").val().indexOf('@', 0) == -1 || $("#mail_cont").val().indexOf('.', 0) == -1 || nombre_cont == "" || mensaje =="") {
						...
						var dataString ='mail_cont='+mail_cont+'&nombre_cont='+nombre_cont+'&mensaje='+mensaje+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>';
					   	$.ajax({
			        		...
			        		success: function(msg){
				 		        if(msg == 1){
									$('#aviso_sus_cont').append("<div class='alert alert-success'><strong>¡Bien hecho!</strong> Ud. se ah suscrito correctamente.</div>");
								}else{
									$('button#contac').removeAttr("disabled");
	...
	<header>
		...
		<div id="ads-cabecera" class="hidden-phone">
			...
			<!--<ins class="adsbygoogle"
			     ...
			     <img src="http://mercadonegro.pe/bw_mn_dep_14-1_tls.png" alt="Publicidad">-->
			<!--<object width="728px">
				<param name="movie" value="<?php echo base_url(); ?>FLASH FIAP BANNERS.swf" >
				<param name="play" value="true" > 
				<embed src="<?php echo base_url(); ?>FLASH FIAP BANNERS.swf" >
				</embed>
			</object>    -->
			<!--<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
			<ins class="adsbygoogle"
			...
			<!--<a href="http://www.somosfiap.pe"><img src="<?php echo base_url(); ?>New/Images/banner somos fiap.png"/></a>
			-->
			<!--<a href="http://www.iabperu.com/ia_digitalday1_2014.aspx">
			<object width="728" height="90">
				<param name="movie" value="<?php echo base_url(); ?>banner.swf" >
				<param name="play" value="true" > 
				<embed src="<?php echo base_url(); ?>banner.swf" >
				</embed>
			</object>
			</a>-
			<a href="http://www.tls.edu.pe/" target="_blank"><img src="<?php echo base_url(); ?>New/Images/bw_ext.jpg" height="90px"/></a>-->
			<object width="728" height="90">
			...
			<div id="navegacion">
				...
				<?php }
					...
					foreach ($importante1 as $impor) {
						$urlBloque1 = noticia::utf8($impor->nombres);
						$urlBloque1 = noticia::urlamigable($urlBloque1);
						if(strlen($impor->bio) > 249){
							$impor->bio = trim(utf8_encode($impor->bio)).'...';
						}
						$urlEspecialista = noticia::urlamigable(utf8_encode($impor->nombres));
							#Controlo que el Cargo no sea muy extenso
							
							$urlNota = noticia::urlamigable(utf8_encode($impor->frase));
					?>
					<div id="importante" class="visible-desktop">
				...
				<?php }
						...
						foreach ($importante2 as $noticia) {
							$urlBloque1 = noticia::utf8($noticia->titulo);
							$urlBloque1 = noticia::urlamigable($urlBloque1);
					?>
					<div id="importante" class="visible-desktop separador">
						...
						<div class="container">
							...
							<div class="nav-collapse collapse">
								<ul class="nav">
									...
									<li ><a href="#suscribir" data-toggle="modal">Suscripción</a></li>
								</ul>
					...
					<?php 
						if(is_array($especialistas)){
							foreach ($especialistas as $especialista) {
								$urlEspecialista = Noticiaerror::urlamigable(utf8_encode($especialista->nombres));
								#Controlo que el Cargo no sea muy extenso
								if(strlen($especialista->cargo) > 50){
									$especialista->cargo = substr(utf8_encode($especialista->cargo), 0, 50).'...';
								}
								$urlNota = Noticiaerror::urlamigable(utf8_encode($especialista->titulocolum));
					?>
					<div class="especialista">
						<img src="<?php echo $especialista->fotoesp; ?>" alt="fw" title=""/>
