...
<head>
	<meta charset="utf-8">
	<?php 
		if(is_array($videomntv)){
			foreach ($videomntv as $video) {
	?>
	<title><?php echo $video->titulo; ?> - MERCADO NEGRO TV</title>
	<?php
			}
		}else{
	?>
	<title>VIDEO NO ENCONTRADO :( - MERCADO NEGRO TV</title>
	<?php
		}
	?>
	...
	<?php foreach ($videomntv as $video) { ?>
	<meta property="og:title" content='<?php echo  $video->titulo; ?>' />
	<meta property="og:description" content='<?php echo $video->titulo; ?> '/>
	<meta property="og:image" content='http://img.youtube.com/vi/<?php echo $video->codyoutube; ?>/0.jpg' />
	<link href="http://img.youtube.com/vi/<?php echo $video->codyoutube; ?>/0.jpg" rel="image_src" />
	<?php
		}
	?>
	<!-- Bootstrap -->
			...
			<div id="navegacion">
				...
				<?php }
					...
					foreach ($importante1 as $impor) {
						...
						$urlBloque1 = videos::urlamigable($urlBloque1);
						if(strlen($impor->bio) > 249){
							$impor->bio = trim($impor->bio).'...';
						}
						$urlEspecialista = videos::urlamigable($impor->nombres);
							#Controlo que el Cargo no sea muy extenso
							
							$urlNota = videos::urlamigable($impor->frase);
					?>
					<div id="importante" class="visible-desktop">
			...
			<div class="span8">
				...
				<section id="masgaleriavideos" class="hidden-phone">
					...
					<?php
						...
						foreach ($videoshome as $videos) {
							$i = $i + 1;
							$url = $videos->titulo;
							$url = Videos::urlamigable($url);
					?>
					<div id="masyoutube">
						<div class="categoria">MERCADONEGRO TV</div>
						<a href="<?php echo base_url().'videos/'.$videos->idnoti.'/'.$url;?>" class="play"></a>
				...
				<?php 
					...
					if(is_array($noticiaBloqueUno)){
						echo '<article id="mercadonegrotv"><h1>NOTICIAS RECIENTES</h1></article>';
						foreach ($noticiaBloqueUno as $bloque1) {
							$urlBloque1 = $bloque1->titulo;
							$urlBloque1 = Videos::urlamigable($bloque1->titulo);
				?>
					...
					<?php 
						if(is_array($especialistas)){
							foreach ($especialistas as $especialista) {
								$urlEspecialista = Videos::urlamigable($especialista->nombres);
								#Controlo que el Cargo no sea muy extenso
								...
								$urlNota = Videos::urlamigable($especialista->titulocolum);
					?>
