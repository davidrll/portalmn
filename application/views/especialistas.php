...
<head>
	<meta charset="utf-8">
	<title>LOS ESPECIALISTAS - MERCADO NEGRO TV</title>
...
<div class="slide_likebox">
	...
	<section id="contenedor">
		...
		<div class="row-fluid">
			<div class="span8">
				<?php 
					if(is_array($columnistas)){
						foreach ($columnistas as $espe) {
							$urlBloque1 = $espe->nombres;
							$urlBloque1 = Especialista::urlamigable($urlBloque1);
							if(strlen($espe->bio) > 249){
								$espe->bio = trim($espe->bio).'...';
							}
							$urlEspecialista = Especialista::urlamigable($espe->nombres);
								#Controlo que el Cargo no sea muy extenso
								
								$urlNota = Especialista::urlamigable($espe->titulocolum);
				?>
				<div id="mininoticia"> 
					<a href="<?php echo base_url().'especialista/'.$espe->codigo.'/'.$urlBloque1;?>"><img src="<?php echo $espe->imag_colum;?>" class="fonoticia2" /></a>
					<!-- COMPARTIR BEGIN -->
					<a class="a2a_dd" href="http://www.addtoany.com/share_save?linkurl=<?php echo base_url().'noticia/'.$espe->codigo.'/'.$urlBloque1;?>&amp;linkname="><img src="<?php echo base_url(); ?>assets/img/share.png" width="20" class="shared" alt="compartir"></a>
					<script type="text/javascript">
						var a2a_config = a2a_config || {};
						a2a_config.linkurl = "<?php echo base_url().'noticia/'.$espe->codigo.'/'.$urlBloque1;?>";
					...
					<h1><a href="<?php echo base_url().'especialista/'.$espe->codigo.'/'.$urlBloque1;?>"><?php echo $espe->nombres; ?></a></h1>
					<span class="exnoticia"><?php echo trim($espe->bio); ?></span>
					<?php 
								if(!empty($espe->titulocolum) && !empty($espe->desc_colum)){
									#Controlo la longitud del Título de la Columna
									/*if(strlen(utf8_encode($espe->titulocolum)) > 60){
										$espe->titulocolum = substr($espe->titulocolum, 0, 60).'...';
									}*/

									if(strlen($espe->desc_colum) > 140){
										$espe->desc_colum = substr($espe->desc_colum, 0, 140).'...';
									}
							?>
					<span class="titulocolum"><a href="<?php echo base_url().'nota/'.$espe->codigo.'/'.$urlNota;?>"><?php echo $espe->titulocolum; ?></a></span>
					<span class="excolumna"><?php echo trim($espe->desc_colum); ?></span>
				</div>
				<?php
								}
						} 
