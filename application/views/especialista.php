...
<head>
	<meta charset="utf-8">
	<?php 
		if(is_array($columnista)){
			foreach ($columnista as $espe) {
				$titulo = $espe->nombres;
	?>
	<title><?php echo $espe->nombres; ?> - MERCADO NEGRO</title>
	<?php
			}
		}else{
	?>
	<title>NO SE ENCONTRÓ AL ESPECIALISTA :( - MERCADO NEGRO TV</title>
	<?php
		}
	?>
	<?php 
			foreach ($columnista as $espe) {
	?>
	<meta property="og:title" content="<?php echo $espe->frase; ?>" />
	<meta property="og:image" content="http://mercadonegro.pe/assets/cdn/logo.png" />
	<?php
		}
	?>
			...
			<div id="navegacion">
				...
				<?php }
					...
					foreach ($importante1 as $impor) {
						...
						$urlBloque1 = especialista::urlamigable($urlBloque1);
						$urlEspecialista = especialista::urlamigable($impor->nombres);
							#Controlo que el Cargo no sea muy extenso
							
							$urlNota = especialista::urlamigable($impor->frase);
					?>
					<div id="importante" class="visible-desktop">
						...
						<div class="container">
							...
							<div class="nav-collapse collapse">
								<ul class="nav">
									...
									<li class="active"><a href="<?php echo base_url(); ?>especialistas/">Especialistas</a></li>
	...
	<section id="contenedor">
		...
		<div class="row-fluid">
			<div class="span8">
				...
				?>
				<div id="mininoticia"> 
					<img src="<?php echo $espe->imag_colum;?>" class="fonoticia" />
					<!-- COMPARTIR BEGIN -->
					...
					<!-- COMPARTIR END -->
					<div class="feregistro"><span class="catcampana">ESPECIALISTA</span></div>
					<h1><?php echo $espe->nombres; ?></h1>
					<span class="exnoticia"><?php echo trim($espe->bio); ?></span>
				</div>
				<div id="mininoticia"> 
					<!-- COMPARTIR BEGIN -->
					<a class="a2a_dd" href="http://www.addtoany.com/share_save?linkurl=<?php echo "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];?>&amp;linkname="><img src="<?php echo base_url(); ?>assets/img/share.png" width="20" class="shared" alt="compartir"></a>
					...
					<script type="text/javascript" src="<?php echo base_url().'assets/js/share.js'?>"></script>
					<!-- COMPARTIR END -->
					<h1><?php echo $espe->frase; ?></h1>
					<span class="exnoticia"><?php echo trim($espe->desc_colum); ?></span>
				</div>
				<div class="fb-comments" data-href="<?php echo "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; ?>" data-numposts="15" data-colorscheme="light"></div>
				<?php
						} 
					} 
				?>
				<!-- NOTAS RELACIONADAS DEL ESPECIALISTAS -->
				<?php 

					if(is_array($notarelac)){
						echo '<article id="mercadonegrotv">
					<h1>NOTAS DEL COLUMNISTA</h1>
				</article>';
						foreach ($notarelac as $rela) {
							$urlrela = $rela->nombres;
							$urlrela = Especialista::urlamigable($rela->nombres);
				?>
				<div id="mininoticia" class="masnoticiascont"> 
					<a href="<?php echo base_url().'especialista/'.$rela->codigo.'/'.$urlrela;?>"><img src="<?php echo $rela->portada;?>" class="fonoticia" /></a>
					<!-- COMPARTIR BEGIN -->
					<a class="a2a_dd" href="http://www.addtoany.com/share_save?linkurl=<?php echo base_url().'especialista/'.$rela->codigo.'/'.$urlrela;?>&amp;linkname="><img src="<?php echo base_url(); ?>assets/img/share.png" width="20" class="shared" alt="compartir"></a>
					<script type="text/javascript">
						var a2a_config = a2a_config || {};
						a2a_config.linkurl = "<?php echo base_url().'especialista/'.$rela->codigo.'/'.$urlrela;?>";
						a2a_config.onclick = 1;
					</script>
					<script type="text/javascript" src="<?php echo base_url().'assets/js/share.js'?>"></script>
					<!-- COMPARTIR END -->
					<div class="feregistro"><span class="catpubli"><?php echo $rela->frase; ?></span> <span class="fechanoti"><?php echo $rela->fecha; ?></span></div>
					<h1><a href="<?php echo base_url().'especialista/'.$rela->codigo.'/'.$urlrela;?>"><?php echo $rela->nombres; ?></a></h1>
					...
					<?php 
						if(is_array($especialistas)){
							foreach ($especialistas as $especialista) {
								$urlEspecialista = Especialista::urlamigable($especialista->nombres);
								#Controlo que el Cargo no sea muy extenso
								...
								$urlNota = Especialista::urlamigable($especialista->titulocolum);
					?>
