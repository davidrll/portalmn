...
<head>
	...
	?>
	<?php if(isset($noticia) && count($noticiamn) > 0){ ?>
		<?php foreach ($noticiamn as $noticia) { ?>
		<meta property="og:title" content='<?php echo  $noticia->titulo; ?>' />
		<meta property="og:description" content='<?php echo  $noticia->extracto; ?> '/>
		<meta property="og:image" content='<?php echo $noticia->sumilla_g; ?>' />
		<link href="<?php echo $noticia->sumilla_g; ?>" rel="image_src" />
		<?php
			}
		?>
	<?php } ?>
...
<script>(function(d, s, id) {
  ...
  js.src = "//connect.facebook.net/es_ES/all.js#xfbml=1&appId=206414429567350";
			...
			<div id="navegacion">
				...
				<?php }
					...
					foreach ($importante1 as $impor) {
						...
						$urlEspecialista = noticia::urlamigable($impor->nombres);
							#Controlo que el Cargo no sea muy extenso
							
							$urlNota = noticia::urlamigable($impor->frase);
					?>
					<div id="importante" class="visible-desktop">
					...
					<div class="navbar-inner">
						<div class="container">
							...
							<div class="nav-collapse collapse">
								<ul class="nav">
									<li class="active"><a href="<?php echo base_url(); ?>noticias/">Noticias</a></li>
		...
		<div class="row-fluid">
			<div class="span8">
				...
				<article id="mercadonegrotv">
					...
					<span class="exnoticia"><?php echo stripslashes(trim($noticia->contenido)); ?></span>
					<div class="fb-comments" data-href="<?php echo "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; ?>" data-width="632" data-numposts="15" data-colorscheme="light"></div>
				</article>
				<?php
						} 
					}else{
				...
				<?php 
					...
					if(is_array($noticiarelac)){
						echo '<article id="mercadonegrotv">
					<h1>NOTICIAS RELACIONADAS</h1>
				</article>';
						foreach ($noticiarelac as $rela) {
							$urlrela = $rela->titulo;
							$urlrela = Noticia::urlamigable($rela->titulo);
				?>
				<div id="mininoticia" class="masnoticiascont"> 
					<a href="<?php echo base_url().'noticia/'.$rela->codigo.'/'.$urlrela;?>"><img src="<?php echo $rela->portada;?>" class="fonoticia" /></a>
					<!-- COMPARTIR BEGIN -->
					<a class="a2a_dd" href="http://www.addtoany.com/share_save?linkurl=<?php echo base_url().'noticia/'.$rela->codigo.'/'.$urlrela;?>&amp;linkname="><img src="<?php echo base_url(); ?>assets/img/share.png" width="20" class="shared" alt="compartir"></a>
					<script type="text/javascript">
						var a2a_config = a2a_config || {};
						a2a_config.linkurl = "<?php echo base_url().'noticia/'.$rela->codigo.'/'.$urlrela;?>";
						a2a_config.onclick = 1;
					</script>
					<script type="text/javascript" src="<?php echo base_url().'assets/js/share.js'?>"></script>
					<!-- COMPARTIR END -->
					<div class="feregistro"><span class="catpubli"><?php echo $rela->subida; ?></span> <span class="fechanoti"><?php echo $rela->fecha; ?></span></div>
					<h1><a href="<?php echo base_url().'noticia/'.$rela->codigo.'/'.$urlrela;?>"><?php echo $rela->titulo; ?></a></h1>
					<span class="exnoticia"><?php echo $rela->extracto; ?>...</span>
				</div>
				<?php
						} 
					} 
				...
				<?php 
					...
					<h1>NOTICIAS RECIENTES</h1>
						...
						foreach ($noticiaBloqueUno as $bloque1) {
							...
							$urlBloque1 = Noticia::urlamigable($bloque1->titulo);
				?>
				<div id="mininoticia" class="masnoticiascont"> 
					<a href="<?php echo base_url().'noticia/'.$bloque1->codigo.'/'.$urlBloque1;?>"><img src="<?php echo $bloque1->portada;?>" class="fonoticia" /></a>
					<!-- COMPARTIR BEGIN -->
					...
					<h1><a href="<?php echo base_url().'noticia/'.$bloque1->codigo.'/'.$urlBloque1;?>"><?php echo $bloque1->titulo; ?></a></h1>
				...
				?>
				<article id="mercadonegrotv" class="hidden-phone">
					<script src="https://apis.google.com/js/platform.js"></script>
					<div class="g-ytsubscribe" data-channel="MercadoNegroTV1" data-layout="full" data-count="default"></div>
				</article>
				<?php if (is_array($videoshome)) { ?>
				<section id="masgaleriavideos" class="hidden-phone">
					...
					<?php
						...
						foreach ($videoshome as $videos) {
							$i = $i + 1;
							$url = $videos->titulo;
							$url = Noticia::urlamigable($url);
					?>
					<div id="masyoutube">
						<div class="categoria">MERCADONEGRO TV</div><a href="<?php echo base_url().'videos/'.$videos->idnoti.'/'.$url;?>" class="play"></a>
						<img src="<?php echo $videos->fotovideo; ?>" />
						<div class="detailvideo"><a href="<?php echo base_url().'videos/'.$videos->idnoti.'/'.$url;?>"><?php echo $videos->titulo; ?></a> </div>
					</div>
					<?php 
						} 
					?>
				...
				<?php } ?>
				<a href="http://www.youtube.com/user/MercadoNegroTV1" target="_blank" id="vermastv" class="btn btn-danger hidden-phone">VER MÁS PROGRAMAS DE MERCADONEGRO TV</a>
				<p></p>
					...
					<?php 
						if(is_array($especialistas)){
							foreach ($especialistas as $especialista) {
								$urlEspecialista = Noticia::urlamigable($especialista->nombres);
								#Controlo que el Cargo no sea muy extenso
								...
								$urlNota = Noticia::urlamigable($especialista->titulocolum);
					?>
